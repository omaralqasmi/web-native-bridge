...
public class NativeWebBridge: NSObject, WKScriptMessageHandler, CLLocationManagerDelegate {
    ...
    public func sendCommandToWeb(action: String, payload: [String: Any]? = nil) {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: req) { dispatchToWeb(base64Payload: data.base64EncodedString()) }
    }
    ...
    private func registerDefaultHandlers() {
        ...
        registerHandler(action: "system.permissions.check") { p, cb in
            ...
            if type == "camera" { cb(AVCaptureDevice.authorizationStatus(for: .video) == .authorized, nil) } 
            else if type == "notifications" { UNUserNotificationCenter.current().getNotificationSettings { settings in DispatchQueue.main.async { cb(settings.authorizationStatus == .authorized || settings.authorizationStatus == .provisional, nil) } } } 
            else { cb(true, nil) }
        }
        registerHandler(action: "system.permissions.request") { p, cb in
            let type = p["type"] as? String ?? ""
            if type == "camera" { AVCaptureDevice.requestAccess(for: .video) { granted in DispatchQueue.main.async { cb(granted, nil) } } } 
            else if type == "notifications" { UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) { granted, error in DispatchQueue.main.async { cb(granted, error?.localizedDescription) } } } 
            else { cb(true, nil) }
        }
        ...
        registerHandler(action: "system.app.clearCaches") { _, cb in DispatchQueue.main.async { let dataStore = WKWebsiteDataStore.default(); let allTypes = WKWebsiteDataStore.allWebsiteDataTypes(); dataStore.removeData(ofTypes: allTypes, modifiedSince: Date(timeIntervalSince1970: 0)) { cb(true, nil) } } }
